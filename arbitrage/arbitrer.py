...
class Arbitrer(object):
    ...
    def init_markets(self, markets):
        ...
        for market_name in markets:
            exec(f'import public_markets.{market_name.lower()}')
            market = eval(f'public_markets.{market_name.lower()}.{market_name}()')
            self.markets.append(market)

    def init_observers(self, _observers):
        self.observer_names = _observers
        for observer_name in _observers:
            exec(f'import observers.{observer_name.lower()}')
            observer = eval(f'observers.{observer_name.lower()}.{observer_name}()')
    ...
    def get_profit_for(self, mi, mj, kask, kbid):
        ...
        max_amount_buy = sum(
            self.depths[kask]["asks"][i]["amount"] for i in range(mi + 1)
        )
        max_amount_sell = sum(
            self.depths[kbid]["bids"][j]["amount"] for j in range(mj + 1)
        )
    ...
    def get_max_depth(self, kask, kbid):
        ...
        if len(self.depths[kbid]["bids"]) != 0 and \
            ...
            while (
                ...
                and i < len(self.depths[kask]["asks"]) - 1
            ):
                i += 1
        j = 0
        if len(self.depths[kask]["asks"]) != 0 and \
            ...
            while (
                ...
                and j < len(self.depths[kbid]["bids"]) - 1
            ):
                j += 1
        return i, j
    ...
    def tickers(self):
        for market in self.markets:
            logging.verbose(f"ticker: {market.name} - {str(market.get_ticker())}")
    ...
    def replay_history(self, directory):
        ...
        files.sort()
        for f in files:
            depths = json.load(open(f'{directory}/{f}', 'r'))
            self.depths = {}
        ...
        for kmarket1 in self.depths:
            for kmarket2 in self.depths:
                ...
                if (
                    ...
                    and float(market1["asks"][0]['price'])
